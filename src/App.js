...
function initAnswers() {
  const a = {};
  MATIERES.forEach(m => {
    a[m.id] = m.items.map(() => ({ note: null, entreprise: false, observation: '' }));
  });
  return a;
}
...
function exportCSV(nom, answers) {
  const BOM = '\uFEFF';
  const header = ['Nom', 'Matiere', 'Item', 'Competence', 'Niveau', 'Pratique_entreprise', 'Observation'];
  const rows = [header.join(';')];

  MATIERES.forEach(m => {
    m.items.forEach((item, idx) => {
      const a = answers[m.id]?.[idx] || {};
      const row = [
        `"${nom}"`,
        `"${m.titre}"`,
        idx + 1,
...
function PageAccueil({ onStart }) {
  ...
  const handleStart = () => {
    if (nom.trim().length < 3) { setErreur(true); return; }
    onStart(nom.trim());
  };
  ...
  return (
    <div className="main-content">
      <div style={{ paddingTop: 16, marginBottom: 32 }}>
        ...
        <h1 className="page-title">Auto-évaluation<br />de compétences</h1>
        <p className="page-subtitle">
          13 matières · {MATIERES.reduce((s, m) => s + m.items.length, 0)} items au total.
          Compter entre 20 et 30 minutes.
        </p>
      </div>

      <div className="card card-disclaimer" style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 12, fontFamily: 'var(--font-mono)', color: 'var(--accent)', marginBottom: 10 }}>
          À lire avant de commencer
        </div>
        <p style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.7 }}>
          Cet outil est strictement pédagogique. Il n'a aucune incidence sur les notes
          ni sur la validation de l'année. Les réponses sont confidentielles et ne sont
          accessibles qu'au class-manager de la promotion. Il n'y a pas de bonne ou de
          mauvaise réponse : l'honnêteté est la seule chose qui rende cet exercice utile.
        </p>
      </div>

      <div className="card" style={{ marginBottom: 24 }}>
        <div className="section-label" style={{ marginBottom: 12 }}>Légende des niveaux</div>
      ...
      <div className="card">
        ...
        <input
          ...
          value={nom}
          onChange={e => { setNom(e.target.value); setErreur(false); }}
...
// PAGE MATIÈRE
// =====================================================================
function PageMatiere({ matiere, matiereIndex, answers, onAnswer, onNext, onPrev, isFirst, isLast }) {
  const matiereAnswers = answers[matiere.id] || [];
  const filled = matiereAnswers.filter(a => a.note !== null).length;
  const total = matiere.items.length;

  const selectedNiveau = (itemIdx) => {
    const note = matiereAnswers[itemIdx]?.note;
    return note ? `sel-${note}` : '';
  };
  ...
  return (
    <div className="main-content">
      <div style={{ marginBottom: 20 }}>
        <div className="section-label">Matière {matiereIndex + 1} sur {MATIERES.length}</div>
        <h2 className="matiere-title">{matiere.titre}</h2>
        <div className="matiere-counter">{filled}/{total} items renseignés</div>
      </div>

      {matiere.items.map((item, idx) => {
        const a = matiereAnswers[idx] || {};
        return (
          <div className={`item-block ${a.note ? 'answered' : ''}`} key={idx}>
            ...
            <div className="rating-grid">
              {NIVEAUX.map(n => (
                <button
                  ...
                  onClick={() => onAnswer(matiere.id, idx, 'note', n.val)}
            ...
            <div
              ...
              onClick={() => onAnswer(matiere.id, idx, 'entreprise', !a.entreprise)}
              role="checkbox"
              aria-checked={a.entreprise}
              tabIndex={0}
              onKeyDown={e => e.key === ' ' && onAnswer(matiere.id, idx, 'entreprise', !a.entreprise)}
            >
            ...
            <textarea
              ...
              value={a.observation || ''}
              onChange={e => onAnswer(matiere.id, idx, 'observation', e.target.value)}
              rows={2}
            />
          </div>
        );
      ...
      <div className="comment-block">
        <div className="comment-block-label">Commentaire général sur cette matière (facultatif)</div>
        <textarea
          className="obs-textarea"
          placeholder="Remarque générale sur la matière..."
          value={answers[`comment_${matiere.id}`] || ''}
          onChange={e => onAnswer(`comment_${matiere.id}`, null, null, e.target.value)}
          rows={3}
        />
      </div>
...
// PAGE BILAN
// =====================================================================
function PageBilan({ nom, answers, onPrev, onReset }) {
  const totalItems = MATIERES.reduce((s, m) => s + m.items.length, 0);
  const totalFilled = MATIERES.reduce((s, m) =>
    s + (answers[m.id] || []).filter(a => a.note !== null).length, 0);
  const totalEntreprise = MATIERES.reduce((s, m) =>
    s + (answers[m.id] || []).filter(a => a.entreprise).length, 0);

  const moyenneGlobale = () => {
    let sum = 0, count = 0;
    MATIERES.forEach(m => {
      (answers[m.id] || []).forEach(a => {
        if (a.note) { sum += a.note; count++; }
      });
    });
  ...
  const moyenneMatiere = (mId) => {
    const items = (answers[mId] || []).filter(a => a.note !== null);
  ...
  return (
    <div className="main-content">
      <div style={{ paddingTop: 16, marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 4 }}>{nom}</p>
      </div>

      <div className="stat-grid">
        <div className="stat-card">
          <span className="stat-num">{totalFilled}/{totalItems}</span>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="section-label" style={{ marginBottom: 16 }}>Score par matière</div>
        {MATIERES.map(m => {
          ...
          return (
            <div className="bilan-matiere" key={m.id}>
              <div className="bilan-matiere-header">
                <span className="bilan-matiere-titre">{m.titre}</span>
      ...
      <div className="card card-disclaimer" style={{ marginBottom: 20 }}>
        <p style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.7 }}>
          Ce bilan n'est visible que sur cet écran. Pour le transmettre au class-manager,
          télécharger le fichier CSV ci-dessous et l'envoyer par email.
        </p>
      </div>
...
export default function App() {
  const [page, setPage] = useState('accueil'); // accueil | matiere | bilan
  ...
  // Correction : charger le nom depuis le storage au montage
  useEffect(() => {
    const saved = loadFromStorage();
    if (saved?.nom) setNom(saved.nom);
    if (saved?.page) setPage(saved.page);
    if (saved?.matiereIndex !== undefined) setMatiereIndex(saved.matiereIndex);
  }, []);
  ...
  const handleReset = () => {
    if (window.confirm('Recommencer effacera toutes les réponses. Confirmer ?')) {
  ...
  const matiereLabel = page === 'matiere'
    ? MATIERES[matiereIndex]?.titre
    : 'Bilan';
  ...
  return (
    <div className="app-shell">
      ...
      {page === 'accueil' && (
        <PageAccueil onStart={handleStart} />
      )}
